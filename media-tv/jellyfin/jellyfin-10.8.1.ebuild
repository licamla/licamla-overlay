...
IUSE="synchronous wal big_cache"
...
src_prepare() {
	...
	sed -i "s|JELLYFIN_FFMPEG_OPT=|#JELLYFIN_FFMPEG_OPT=|" "debian/conf/${PN}" \
		|| die
    if !(use synchronous) ; then
		sed -i 's|Synchronous => null|Synchronous => 0|' "Emby.Server.Implementations/Data/BaseSqliteRepository.cs" \
        || die
	fi

    if use wal ; then
        sed -i "s|TRUNCATE|WAL|" "Emby.Server.Implementations/Data/BaseSqliteRepository.cs" \
            || die
    fi

    if use big_cache; then
        sed -i "s|protected virtual int? CacheSize => null|protected virtual int? CacheSize => 10000|" "Emby.Server.Implementations/Data/BaseSqliteRepository.cs" \
            || die
    fi
}
